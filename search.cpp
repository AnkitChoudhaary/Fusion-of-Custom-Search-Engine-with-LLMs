...
{
private:
    std::vector<pair<double, Node *>> heap;

public:
    void insert(double score, Node *rt)
    {
        heap.push_back({score, rt});
        heapifyUp(heap.size() - 1);
    }
    void Insert_2(double score, Node *rt)
    {
...
SearchEngine::SearchEngine()
{
    ...
    ip.close();
    ip.open("./txtfiles/english.txt");
    if (ip.is_open())
    {
        string p;
        while (std::getline(ip, p))
        {
            stop_words.insert_sentence(1, p);
        }
    }
    ip.close();
}
...
{
    if (p)
        cout << "\n\nThe Query that we are processing after removing stop words: \n               ";
    if (n_matches == 0)
        return NULL;
    ...
    for (size_t i = 0; i < pattern.size(); i++)
    {
        ...
        if (str.size() > 0)
        {
            ...
            {
                query.push_back(str);
                double sc = double(1 + word_ct.get_word_count(str)) / double(1 + gen_word_freq.get_word_count(str));
                score.push_back(sc);
            }
            else
            {
                if (!stop_words.get_word_count(str))
                {
                    cout << str << " ";
                    query.push_back(str);
                }
...
vector<double> SearchEngine::relavency_q(vector<string> q)
{
    vector<double> ans;
    double num_words = 17215668;
    for (size_t i = 0; i < q.size(); i++)
    {
        double ctt = word_ct.get_word_count(q[i]);
        double k = log10(num_words / (ctt + 1));
        ans.push_back(k);
    }
    return ans;
}